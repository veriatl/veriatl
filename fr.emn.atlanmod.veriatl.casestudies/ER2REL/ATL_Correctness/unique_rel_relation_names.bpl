...
forall s:ref :: {read($tarHeap,s,alloc)}
  Seq#Contains(Fun#LIB#AllInstanceFrom($tarHeap, REL$RELSchema), s)
     ==> 
(

forall r1,r2:ref, __r1,__r2:int :: 
  0<= __r1 && __r1  < __r2 && __r2  <_System.array.Length(read($tarHeap, s, REL$RELSchema.relations)) &&
     $Unbox(read($tarHeap, read($tarHeap, s, REL$RELSchema.relations), IndexField(__r1))): ref == r1  &&  $Unbox(read($tarHeap, read($tarHeap, s, REL$RELSchema.relations), IndexField(__r2))): ref == r2 
      ==> r1 != r2 ==> read($tarHeap, r1, REL$Relation.name) != read($tarHeap, r2, REL$Relation.name)
