...
procedure T2TB_applyAll() returns();
 ...
 read($srcHeap, t1, HSM$Transition.source) == src && read($srcHeap, t1, HSM$Transition.target) == trg && read($srcHeap, c, HSM$AbstractState.compositeState) == src && !(dtype(trg) == HSM$CompositeState) && !(c == src)  ==>
