...
forall s:ref :: {read($tarHeap,s,alloc)}
  Seq#Contains(Fun#LIB#AllInstanceFrom($tarHeap, FSM$AbstractState), s)
     ==> read($tarHeap, t, FSM$Transition.target) == s ==> 
(

forall sm1,sm2:ref :: {read($tarHeap,sm1,alloc),read($tarHeap,sm2,alloc)}
  Seq#Contains(Fun#LIB#AllInstanceFrom($tarHeap, HSM$StateMachine), sm1) && Seq#Contains(Fun#LIB#AllInstanceFrom($tarHeap, HSM$StateMachine), sm2)
     ==> read($tarHeap, t, FSM$Transition.stateMachine) == sm1 && read($tarHeap, s, FSM$AbstractState.stateMachine) == sm2 ==> sm1 == sm2
