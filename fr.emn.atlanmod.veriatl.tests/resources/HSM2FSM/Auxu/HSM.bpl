/********************************************************************
	@name Boogie declarations and axioms for metamodel HSM
...
axiom (forall __r: ref :: dtype(__r) != HSM$AbstractState);
...
function valid_src_model($h: HeapType): bool{
	...
	(forall __initialstate : ref ::
	  (__initialstate!=null && read($h, __initialstate, alloc) && dtype(__initialstate) <: HSM$InitialState ==> 
	    true
	  )
	)&&
	(forall __regularstate : ref ::
	  (__regularstate!=null && read($h, __regularstate, alloc) && dtype(__regularstate) <: HSM$RegularState ==> 
	    true
	  )
	)&&
