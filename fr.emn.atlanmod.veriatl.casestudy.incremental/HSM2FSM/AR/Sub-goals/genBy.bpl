...
axiom (forall __r: ref, $s: HeapType, $t: HeapType :: genBy(__r, _SM2SM, $s, $t) <==>
Seq#Index(getTarsBySrcs_inverse(__r), 0)!=null && read($s,Seq#Index(getTarsBySrcs_inverse(__r), 0),alloc) && dtype(Seq#Index(getTarsBySrcs_inverse(__r), 0))<:HSM$StateMachine &&
true
);
...
axiom (forall __r: ref, $s: HeapType, $t: HeapType :: genBy(__r, _CS2RS, $s, $t) <==>
Seq#Index(getTarsBySrcs_inverse(__r), 0)!=null && read($s,Seq#Index(getTarsBySrcs_inverse(__r), 0),alloc) && dtype(Seq#Index(getTarsBySrcs_inverse(__r), 0))<:HSM$CompositeState &&
true
);
