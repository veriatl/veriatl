...
class mm2boogie {
	...
	def static dispatch gen_Classifiers(EClass eClass) '''
		...
		const unique «mmName»$«eClass.name»: ClassName extends «(0..<eClass.ESuperTypes.size).map(i | mmName + "$" + eClass.ESuperTypes.get(i).name).join(",")» complete;
		«FOR sf : eClass.EStructuralFeatures»
			«gen_StructuralFeatures(eClass, sf)»	
		«ENDFOR»
		  «if (eClass.abstract) String.format("axiom (forall r: ref :: dtype(r)!= %s$%s);", mmName, eClass.name)»
	'''
