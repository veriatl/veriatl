...
forall a:ref, __a:int :: 
  0<= __a && __a  <_System.array.Length(read($tarHeap, r, REL$Relation.attrs)) &&
     $Unbox(read($tarHeap, read($tarHeap, r, REL$Relation.attrs), IndexField(__a))): ref == a 
      ==> !(read($tarHeap, a, REL$RELAttribute.isKey))
